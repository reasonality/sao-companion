...
export async function callAcquisitionSpecialist(narrativeText) {
    ...
    const systemPrompt = `你是 SAO 游戏的获取事件检测器。阅读叙事文本，识别所有**获取事件**并输出对应的获取标签。

## 检测规则
1. 只在叙事明确描写了**获取/获得/领悟/学会/加入/创建/使用**时才输出标签
2. **卖出/出售/卖掉/交易给NPC/丢失/被夺/赠予/销毁 → 不输出任何标签**（这些不是获取事件，由状态专家处理）
3. 没有获取事件则不输出任何标签（输出空文本）
4. 每个获取事件输出一个标签，每个标签占一行
5. 不要输出任何其他文本（不要解释、不要注释、不要 markdown、不要代码块标记）
6. 购买/买入/从商店获得 → 输出 <gain_equipment> 或对应类型的 gain 标签（购买=获得）
